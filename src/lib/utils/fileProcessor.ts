...
export async function processImage(file: File, rows: number, cols: number) {
  ...
  try {
    ...
    cv.adaptiveThreshold(grayMat, threshMat, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 57, 5);
    grayMat.delete();
    ...
    if (corners.length < 4) {
      ...
      const cornersMat = cv.matFromArray(corners.length, 1, cv.CV_32SC2, 
    ...
    const cropped_image = rotated_image.roi(new cv.Rect(top_left.x, top_left.y, bottom_right.x - top_left.x, bottom_right.y - top_left.y));
